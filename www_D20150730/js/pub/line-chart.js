/* 自定义linechart的实现 */
//绘制背景的若干平行线 给定位置和总条数
app.factory('lineChartService',function(){
...
//绘制折线图 需要接受state来传递路由信息
    var drawLineChart=function(data,$state,communicateService,paramsService,$rootScope){
        ...
        //增加点击事件
        if(!$rootScope.isFirstLineChart){
            canvas.addEventListener('click',clickPoint,true);
            $rootScope.isFirstLineChart=true;
        }
//        canvas.removeEventListener('click',clickPoint,true);
        function clickPoint(e){
            ...
            var tempDate=new Date(data.currentDate);
            for(var i=0;i<dataPoints.length;i++){
                var distance=Math.sqrt(Math.pow(clickX-dataPoints[i].x,2)+Math.pow(clickY-dataPoints[i].y,2));
                if(distance<16){
                    var billingCycle;
                    if(data.indexCycle=='M'){
                        billingCycle=new Date(tempDate.setMonth(tempDate.getMonth()-6+i));
                        tempDate=new Date(data.currentDate);
                    }else if(data.indexCycle=='D'){
                        billingCycle=new Date(tempDate.setDate(tempDate.getDate()-6+i));
                        tempDate=new Date(data.currentDate);
                    }
                    var content={
                        ...
                        "billingCycle":billingCycle.getFullYear()+(billingCycle.getMonth()<9?'0':'')+(billingCycle.getMonth()+1)+(billingCycle.getDate()<10?'0':'')+(billingCycle.getDate())
