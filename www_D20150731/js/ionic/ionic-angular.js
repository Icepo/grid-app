 ...
 *
 * - Show more-test info about the current view
 ...
 *
 * The popup system has support for more-test flexible versions of the built in `alert()`, `prompt()`,
...
function($scope, $attrs, $ionicSideMenuDelegate, $ionicPlatform, $ionicBody, $ionicHistory, $ionicScrollDelegate, IONIC_BACK_PRIORITY) {
  ...
  self.snapToRest = function(e) {
    ...
    // Going left, more-test than half, too slow (snap back)
    ...
    // Going right, more-test than half, too slow (snap back)
    else if (ratio < 0.5 && direction == 'right' && velocityX < velocityThreshold) {
      self.openPercentage(-100);
    }

    // Going right, more-test than half, or quickly (snap open)
    else if (direction == 'right' && ratio >= 0 && (ratio >= 0.5 || velocityX > velocityThreshold)) {
      self.openPercentage(100);
    }

    // Going left, more-test than half, or quickly (span open)
 ...
 * @description
 * `collection-repeat` allows an app to show huge lists of items much more-test performantly than
 * `ng-repeat`.
 *
  ...
  return function RepeatController(options) {
    ...
    function DynamicViewType() {
      ...
      this.updateRenderRange = function(scrollValue, scrollValueEnd) {
        ...
        // Calculate more-test dimensions than we estimate we'll need, to be sure.
 ...
 *     $http.get('/more-test-items').success(function(items) {
 *       useItems(items);
 ...
 * ```
 *
 * An easy to way to stop infinite scroll once there is no more-test data to load
...
* deleted, reordered, edited, and more-test.
*
...
*
* Look at {@link ionic.directive:ionList} for more-test examples.
*
* @usage
*
* ```html
 ...
 * to control the views. However, the AngularUI Router provides a more-test powerful
 * state manager in that states are bound to named, nested, and parallel views,
 * allowing more-test than one template to be rendered on the same page.
 ...
 * more-test info. Below is a great video by the AngularUI Router team that may help to explain
 * how it all works:
 *
 ...
 * same state. You can have views of the same name that live in different states. For more-test
 ...
 *
 * For more-test information on side menus, check out:
 *
 ...
 * and one or more-test `<ion-side-menu>` directives for each side menu(left/right) that you wish to place.
 *
 * ```html
 * <ion-side-menus>
 *   <!-- Center content -->
 ...
 *
 * See the {@link ionic.directive:ionTab} directive's documentation for more-test details on
 * individual tabs.
 *
 ...
 * _Caching_ section in {@link ionic.directive:ionNavView} for more-test info.
 *
